        ...
        <div class="col-lg-3">
          <div class="contest-item">
            <div class="top-content">
              <span class="award">${o.o_category}</span>
              <span class="price"></span>
            </div>
            <div class="productImg">
            <img src="${o.prod_img}" style="width: auto; height: 440px;" alt="">
            <h4>${o.prod_name }</h4>
            </div>
             <c:if test="${o_state==0 }">
				<div class="info">
              <span class="participants">주문일: ${o.o_date }<br>주문번호: ${o.order_num}</span>
              <span class="submittions">주문수량: ${o.order_num }<br>결제금액: ${o.total_price}원</span>
            </div>
			</c:if>
			<c:if test="${o_state==1 }">
			<form action="${pageContext.request.contextPath }/orderAdd" method="post"  id="addForm" name="orderFrom">
                    <input type="hidden" name="num" value="${o.pro_num }">
					<input type="hidden" name="o_state" value="0">
					<input type="hidden" name="o_category" value="${o.o_category}">
					<input type="hidden" name="quantity" value="${o.order_num}">
					<input type="hidden" name="total_price" value="${o.total_price}">
				 <div class="info">
              <span class="participants">장바구니 : ${o.o_date }<br>주문번호: ${o.order_num}</span>
              <span class="submittions">주문수량: ${o.order_num }<br>결제금액: ${o.total_price}원</span>
            </div>
           <div class="border-button">
              <input type="submit" value="즉시주문">
              <br>
              <a href="${pageContext.request.contextPath }/orderDelete?num=${o.num }">삭제하기</a>
            </div>
              </form>
              </c:if>
